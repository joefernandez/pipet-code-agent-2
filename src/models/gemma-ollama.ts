...
export async function gemmaServiceRequest(requestText: String) {
  ...
  const gemmaHost = vscode.workspace.getConfiguration().get<string>('gemma.ollama.host');
  if (!gemmaHost) {
  ...
  const options = {
    ...
    port: 11434, // default Ollama web service port number
    path: '/api/generate', // web service endpoint path
    method: 'POST',
    headers: {
    ...
    const req = http.request(options, (res: any) => {
      ...
      res.on('end', () => {
        try {
          ...
          resolve(jsonResponse.response); // Resolve the Promise
        } catch (error) {
          reject(error);
        }
      });
